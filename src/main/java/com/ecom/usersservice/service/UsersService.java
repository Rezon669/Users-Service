...
import com.ecom.usersservice.feignclient.JwtServiceFeignClient;
...
public class UsersService {
	...
	@Autowired
	JwtServiceFeignClient jwtTokenService;
	...
	public UsersService(UsersRepository usersRepository, EmailService emailService, PasswordEncoder passwordEncoder,
	...
	/*
	 * public String loginValidation(String email, String password) throws
	 * CustomException { String jwtToken = null; if (email.isEmpty() ||
	 * password.isEmpty()) { logger.warn("Please enter the Emailid & Password");
	 * throw new CustomException("Please enter the Emailid & Password"); }
	 * 
	 * if (usersRepository.findByEmailid(email).isEmpty()) {
	 * logger.warn("EmailID doesn't exist"); throw new
	 * CustomException("EmailID doesn't exist"); }
	 * 
	 * try { String role = usersRepository.findEmailid(email);
	 * UsernamePasswordAuthenticationToken auth = new
	 * UsernamePasswordAuthenticationToken(email, password);
	 * loginDetails.setEmailid(email); // System.out.println(email); Authentication
	 * authentication = authenticationManager.authenticate(auth);
	 * 
	 * if (authentication.isAuthenticated()) { // jwtToken =
	 * jwtService.generateToken(email, role);
	 * 
	 * try { jwtToken = jwtTokenService.generateToken(email, role); } catch
	 * (Exception e) { logger.error("JWT service is not available", e); throw new
	 * CustomException("JWT service is not available"); }
	 * 
	 * 
	 * 
	 * } } catch (Exception e) { System.out.println(e); throw new
	 * CustomException("Getting some error..."); }
	 * 
	 ...
	 * 
	 * // return setEncryptedToken(jwtToken); return jwtToken; }
	 */
	...
	public String loginValidation(String email, String password) throws CustomException {
		...
		try {
			...
			throw new CustomException("Getting some error...! when generating the token");
		}
